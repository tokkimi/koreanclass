...
export default function Vocabulary() {
 ...
 return <div className="container page"><div className="page-head"><h1>Le vocabulaire en photos</h1><p className="muted">Observe, écoute et retiens le mot dans une phrase. Cache les traductions pour tester ta mémoire.</p></div>
 <section className="card"><div className="row" style={{flexWrap:'wrap'}}><label>Thème <select className="input" value={category} onChange={e=>setCategory(e.target.value)}>{['Tout','Manger & boire','Animaux & nature','Au quotidien'].map(c=><option key={c}>{c}</option>)}</select></label><label>Rechercher <input className="input" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Français, coréen ou romanisation" /></label><button className="btn ghost" aria-pressed={hidden} onClick={()=>{setHidden(!hidden);setRevealed([])}}>{hidden?'Afficher les traductions':'Cacher les traductions'}</button></div></section>
 <p className="muted">{visible.length} mots · Photos : Unsplash</p><div className="level-grid">{visible.map(([cat,ko,rom,fr,sentence,translation,photo])=><article className="card" key={ko}><img src={`https://images.unsplash.com/${photo}?auto=format&fit=crop&w=640&q=80`} alt={hidden?`Illustration à reconnaître : ${cat}`:fr} loading="lazy" style={{width:'100%',height:210,objectFit:'cover',borderRadius:18}}/><p className="small muted">{cat}</p><h2 lang="ko">{ko} <SpeakButton text={ko}/></h2><p className="muted">{rom}</p>{!hidden||revealed.includes(ko)?<p><strong>{fr}</strong></p>:<button className="btn ghost" onClick={()=>setRevealed([...revealed,ko])}>Révéler le sens</button>}<p lang="ko">{sentence} <SpeakButton text={sentence}/></p>{(!hidden||revealed.includes(ko))&&<p>{translation}</p>}</article>)}</div>{!visible.length&&<p>Aucun mot trouvé. Essaie un autre mot ou un autre thème.</p>}</div>
}
